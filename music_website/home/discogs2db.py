...
def save_artist_to_db(discogs_id) -> int:
    try:
        ...
        try:
            ...
            db_art = Artist(discogs_artist_id=discogs_id, name=artist.title,
                            bio=artist.bio, picture_url=artist.cover_image)
            db_art.save()
        except:
...
def save_record_to_db(discogs_release_id) -> int:
    try:
        ...
        try:
            record = discogs.get_record_master_by_id(
                discogs_release_id, include_tracklist=False, include_videos=False)
            artist_id = save_artist_to_db(record.artist.discogs_id)
            db_rec = Record(artist_id=Artist.objects.get(pk=artist_id), discogs_release_id=discogs_release_id,
                            title=record.title, year=record.year, cover_url=record.cover_image)
            db_rec.save()
        except:
